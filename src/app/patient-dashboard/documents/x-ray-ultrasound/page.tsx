...
const XRayUltraSound = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "RESI")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="ultraSound">
          ...
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`}avez pas encore enregistré de Radio ou Radio ou scanner ou IRM.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
                    ...
                    return (
                      <tbody key={index}>
                        <tr className="cursor-pointer text-[2vw] md:text-[6px] xl:text-2xs 2xl:text-xs font-bold lg:border-b lg:border-slate-100">
                          ...
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left  hidden lg:table-cell">
                            <DynamicHtmlTag type="span">{item?.details?.partExamined?.name}</DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left hidden lg:table-cell">
                            <DynamicHtmlTag type="span">{item?.details?.laterality?.name}</DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left">
                            <DynamicHtmlTag type="span">{item?.date}</DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left w-1/6">
                            <DynamicHtmlTag type="div" className="flex items-center space-x-2 lg:space-x-4 justify-center">
                              ...
                              <CustomButton
                                onClick={() => handleUpdateCategoryList(item?.id)}
                                as="button"
                                title={
                        ...
                        <tr className="md:hidden border-b border-slate-100 pb-1">
                            ...
                            <DynamicHtmlTag
                              ...
                              <DynamicHtmlTag type="div" className="flex items-center justify-between w-full">
                                ...
                                <DynamicHtmlTag type="div" className="w-1/3">
                                  ...
                                  <DynamicHtmlTag type="span" className=" font-semibold text-black">
                                    {item?.details?.partExamined?.name}
                                  </DynamicHtmlTag>
                                </DynamicHtmlTag>
                                <DynamicHtmlTag type="div" className="w-1/3">
                                  <DynamicHtmlTag type="div" className="text-gray-500">
      ...
      <DeleteDocument
        ...
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer cette radio, écho, scanner, irm : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des radios, échos, scanners, irms ?"
        }
        icon={<FaTrash className="text-red-500" />}
        title={MedicalDocumentName ? "Supprimer une Radio, écho, scanner, IRM" : "Supprimer les radios, échos, scanners, irms"}
