...
const PreventionScreen = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "PIDE")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="prevention-screen">
      ...
      <DynamicHtmlTag type="div" className="flex lg:hidden items-center justify-between mt-2 pb-4 lg:border-b border-slate-100 mx-4">
        <CustomButton onClick={handleaddCategoryList}>
          <CustomLink
            ...
            className="card-btn text-2xs md:text-3xs text-white py-2 px-3 font-semibold rounded-full lg:hidden">
            Ajouter un document
          </CustomLink>
        </CustomButton>
          ...
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`}avez pas encore enregistré de document relatif à la prévention ou au dépistage.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
          <DynamicHtmlTag type="div" className="max-h-table overflow-auto min-w-full bg-white">
            <DynamicHtmlTag type="div" className="flex justify-between px-3 lg:mt-2">
                      ...
                      <tbody key={index}>
                        <tr className="cursor-pointer text-[2vw] md:text-[6px] xl:text-2xs 2xl:text-xs font-bold lg:border-b lg:border-slate-100">
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left">
                            <DynamicHtmlTag type="div" className="flex gap-0 lg:gap-5">
                        ...
                        <tr className="md:hidden pb-1 border-b border-slate-100">
                          <td colSpan={4} className="pb-2">
                            <DynamicHtmlTag
                              type="div"
                              className="bg-gray-400 bg-opacity-40 rounded-lg text-[2vw] md:text-[6px] font-bold w-full px-3 gap-4">
      ...
      <DeleteDocument
        ...
        title={MedicalDocumentName ? "Supprimer une prévention et dépistage" : "Supprimer les préventions et dépistages"}
        description={`Êtes-vous sûr de vouloir supprimer \n ?`}
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer cette prévention et dépistage : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des préventions et dépistages ?"
        }
      />
    </DynamicHtmlTag>
  );
