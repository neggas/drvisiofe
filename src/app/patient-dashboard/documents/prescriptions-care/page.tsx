...
const PrescriptionsCare = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "ORDO")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="prescriptions-care">
        ...
        {CategoryList.length === 0 ? (
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`}avez pas encore enregistré d{`'`}ordonnances et soins.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
          <DynamicHtmlTag type="div" className="max-h-table overflow-auto min-w-full bg-white">
            ...
            <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
              <DynamicHtmlTag
                type="div"
                className="max-h-table overflow-auto min-w-full bg-white lg:h-[88%] lg:max-h-[15rem] xl:max-h-[32rem] table-mobile-height pb-11 lg:pb-0">
                    ...
                    return (
                      <tbody key={index}>
                        <tr className="cursor-pointer text-[2vw] md:text-[6px] xl:text-2xs 2xl:text-xs font-bold lg:border-b lg:border-slate-100">
                          ...
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 hidden lg:block ">
      ...
      <DeleteDocument
        ...
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer cette ordonnance et soins : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des ordonnances et soins ?"
        }
        icon={<FaTrash className="text-red-500" />}
        title={MedicalDocumentName ? "Supprimer une ordonnance et soins" : "Supprimer les ordonnances et soins"}
...
export default PrescriptionsCare;
