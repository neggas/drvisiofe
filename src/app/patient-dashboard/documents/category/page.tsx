...
import {
  selectMedicalDocumentCategoryID,
  selectMedicalDocumentCategoryList,
  selectMedicalDocumentUpdateCategoryID,
...
const DocumentTabs: React.FC = () => {
  ...
  const [docMedicalId, sedocMedicalId] = useState<number | null>(null);
  ...
  const fetchMedicalDocumentCategoryList = useSelector(selectMedicalDocumentCategoryList);
  const fetchMedicalDocumentCategoryID = useSelector(selectMedicalDocumentCategoryID);
  const fetchMedicalDocumentUpdateCategoryID = useSelector(selectMedicalDocumentUpdateCategoryID);
  ...
  const getDetailsSelectCategory = () => {
    if (!hasFetchedCategory.current && fetchMedicalDocumentCategoryID && defaultDocumentCategoryOptions.length > 0) {
      const categoryOptions = defaultDocumentCategoryOptions.find(category => category.value === fetchMedicalDocumentCategoryID);

      if (categoryOptions) {
        setmedicalDocument(prevState => ({
          ...prevState,
          category: categoryOptions, // Directly set the matching category object
        }));
        fetchTTypeDocumentList(categoryOptions.value);
        setSelectCategoryType(categoryOptions.code);
      }
      hasFetchedCategory.current = true; // Set flag to true after the first fetch
    }
  };

  const getCategoryDetailsByUpdate = () => {
    if (!hasFetchedCategory.current && fetchMedicalDocumentUpdateCategoryID && fetchMedicalDocumentCategoryList.length > 0) {
      const categoryOptions = fetchMedicalDocumentCategoryList
        .filter((category: any) => category.id === fetchMedicalDocumentUpdateCategoryID)
        .map((category: any) => category);

      if (categoryOptions) {
        sedocMedicalId(categoryOptions[0]?.id);
        setmedicalDocument(prevState => ({
          ...prevState,
          category: {
            value: categoryOptions[0]?.category.id, // Assigning the `id` as `value`
            label: categoryOptions[0]?.category.name, // Assigning the `name` as `label`
            code: categoryOptions[0]?.category.code, // Including `code` in the category
          },
          typeId: categoryOptions[0]?.type.id,
          laterality: categoryOptions[0]?.details?.laterality?.id,
          partExamined: categoryOptions[0]?.details?.partExamined?.id,
          consultationTypeId: categoryOptions[0]?.details?.consultationType?.id,
          file: categoryOptions[0]?.file,
          date: categoryOptions[0]?.date,
          name: categoryOptions[0]?.name?.replace(/\.[^/.]+$/, ""),
        }));
        setFileType(categoryOptions[0]?.file?.extension);
        //  setPreviewImageDocuments(`${process.env.NEXT_PUBLIC_API_BASE_URL}${categoryOptions[0]?.file.url}`)
        if (categoryOptions[0]?.file?.extension === "application/pdf") {
          const fullUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}${categoryOptions[0]?.file.url}`;
          fetch(fullUrl)
            .then(response => {
            ...
            .then(blob => {
              ...
              setPreviewImageDocuments(objectUrl); // Use object URL for preview
            });
        }
        if (
          categoryOptions[0]?.file?.extension === "application/msword" ||
          categoryOptions[0]?.file?.extension === "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        ) {
          const fullUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}${categoryOptions[0]?.file.url}`;
        ...
        if (categoryOptions[0]?.date) {
          const [day, month, year] = categoryOptions[0]?.date.split("/").map(Number);
          const parsedDate = new Date(year, month - 1, day);
          if (!isNaN(parsedDate.getTime())) setStartDate(parsedDate);
        }
        fetchTTypeDocumentList(categoryOptions[0]?.category.id);
        setSelectCategoryType(categoryOptions[0]?.category?.code);
      }
    }
  };
  useEffect(() => {
    getCategoryDetailsByUpdate();
  }, [fetchMedicalDocumentUpdateCategoryID, fetchMedicalDocumentCategoryList]);
  useEffect(() => {
    getDetailsSelectCategory();
  }, [fetchMedicalDocumentCategoryID, defaultDocumentCategoryOptions]);
  ...
  const handleUpdateDocumentSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    try {
      // Validate the form data
      // Prepare FormData
      const formData = new FormData();
      if (fetchMedicalDocumentUpdateCategoryID) {
        formData.append("docMedicalId", docMedicalId?.toString() || ""); // Default to empty string if null
      }
      formData.append("patientId", fetchPatientData?.id);
      formData.append("categoryId", medicalDocument.category?.value);
      formData.append("name", medicalDocument.name);
      if (medicalDocument.partExamined) {
        formData.append("partExaminedId", medicalDocument.partExamined);
      }
      if (medicalDocument.laterality) {
        formData.append("lateralityId", medicalDocument.laterality);
      }
      formData.append("date", getFormateDatetwoDateformate(medicalDocument.date));
      formData.append("typeId", medicalDocument.typeId);
      if (medicalDocument.consultationTypeId) {
      ...
      if (medicalDocument.file.url) {
        await appendFileIfExists("file", medicalDocument.file.url);
      } else {
      ...
      if (fetchMedicalDocumentUpdateCategoryID) {
        await UpdateMedicalDocument(formData);
        if (fileResponse) {
          await deleteDocumentFile(fetchPatientData?.id, fileResponse?.id);
        }
      } else {
          ...
          <DynamicHtmlTag
            ...
            <DynamicHtmlTag
              ...
              <HeadingTag type="h2" className="text-xs 2xl:text-sm font-semibold py-1 lg:py-0 mt-2 lg:mt-3">
                {fetchMedicalDocumentUpdateCategoryID ? "METTRE À JOUR UN DOCUMENT" : "AJOUTER UN DOCUMENT"}
              </HeadingTag>
              <DynamicHtmlTag type="div" className="text-end lg:hidden px-2 mt-2">
                ...
                <DynamicHtmlTag type="div" className="overflow-auto h-full">
                  ...
                  <DynamicHtmlTag type="div" className="hidden lg:flex mt-2 ps-2 pe-1 lg:px-2 items-center">
                    ...
                    fileType === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ? (
                      <DynamicHtmlTag type="div" className="flex gap-0.5 items-center">
                        ...
                        <CustomButton
                          ...
                          onClick={handleButtonClick}>
                          {fetchMedicalDocumentUpdateCategoryID ? "Modifier" : "Importer"}
                        </CustomButton>
                      </DynamicHtmlTag>
                    ) : (
                      ""
                    ...
                    <DynamicHtmlTag type="span" className="text-3xs md:text-2xs lg:text-xs 2xl:text-sm font-semibold flex justify-end w-full">
                      {fetchMedicalDocumentUpdateCategoryID ? "Mettre à jour un fichier (pdf, jgp, png)" : "Ajoutez un fichier (pdf, jgp, png"}
                    </DynamicHtmlTag>
                  </DynamicHtmlTag>
                  ...
                  <DynamicHtmlTag type="div" className="mt-2 ps-2 pe-1 lg:px-2">
                    ...
                    <DynamicHtmlTag type="span" className="text-2xs 2xl:text-xs font-semibold">
                      ...
                      <CustomSelect
                        ...
                        onChange={(selectedOption: DocumentOption | null) => {
                          ...
                          setErrors((prevErrors: any) => ({
                            ...prevErrors,
                            category: undefined, // Clear the error for category
                          }));
                        }}
                        // isClearable
