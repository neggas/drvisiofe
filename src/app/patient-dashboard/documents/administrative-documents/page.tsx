...
const AdministrativeDocument = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "PIAD")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="administer">
        <HeadingTag type="h3" className="font-semibold text-xs lg:text-2xs 2xl:text-xs text-white text-center py-1 px-3">
          Pièces administratives
        </HeadingTag>
      </DynamicHtmlTag>
      <DynamicHtmlTag type="div" className="flex lg:hidden items-center justify-between mt-2 pb-4 lg:border-b border-slate-100 mx-4 bg-base-100">
        <CustomButton
          as="button"
          ...
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`}avez pas encore enregistré de pièces administratives.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
      ...
      <DeleteDocument
        ...
        title={MedicalDocumentName ? "Supprimer une pièce administrative" : "Supprimer les pièces administratives"}
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer cette pièce administrative : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des pièces administratives ?"
        }
        // paragraph={`Êtes vous sur de vouloir supprimer ce pièces administratives : ${MedicalDocumentName} ?`}
