...
const Résultatdebiologie = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "REBI")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="biology">
          ...
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`}avez pas encore enregistré de résultat de biologie.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
                    ...
                    return (
                      <tbody key={index}>
                        <tr className="cursor-pointer text-[2vw] md:text-[6px] xl:text-2xs 2xl:text-xs font-bold lg:border-b lg:border-slate-100">
                          ...
                          <td className="py-1 px-1 lg:py-3 lg:px-3 hidden lg:block">
                            <DynamicHtmlTag type="span" className="flex items-center line-clamp-1">
                              {item?.name}
                            </DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 lg:py-3 lg:px-3 text-left">
                            <DynamicHtmlTag type="span" className="line-clamp-1">
                              {item?.date}
                            </DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 lg:py-3 lg:px-3 w-1/6">
        ...
        <CustomModal
          ...
          <DynamicHtmlTag type="div" className="modal-box bg-gradient-to-l from-sky-500 to-indigo-500 p-0 pt-4">
            <DynamicHtmlTag type="div" className="bg-base-100 p-4">
              <HeadingTag type="h3" className="text-blue text-sm md:text-lg flex items-center gap-2 font-semibold">
                Tous mes documents
                <MdClose
      ...
      <DeleteDocument
        ...
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer ce résultat de biologie : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des résultats de biologie ?"
        }
        icon={<FaTrash className="text-red-500" />}
        title={MedicalDocumentName ? "Supprimer un résultat de biologie" : "Supprimer les résultats de biologie"}
...
export default Résultatdebiologie;
