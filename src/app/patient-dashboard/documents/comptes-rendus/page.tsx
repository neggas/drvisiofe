...
const DocumentView = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "CORE")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  // }
  return (
    <DynamicHtmlTag type="div" className="lg:shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="document-view">
        <HeadingTag type="h3" className="font-semibold text-xs lg:text-2xs 2xl:text-xs text-white text-center py-1 px-3">
          Comptes rendus
        </HeadingTag>
      </DynamicHtmlTag>
      <DynamicHtmlTag type="div" className="flex lg:hidden items-center justify-between mt-2 pb-4 lg:border-b border-slate-100 mx-4">
        <CustomButton as="div" onClick={handleaddCategoryList}>
          <CustomLink
        ...
        <CustomLink href="/patient-dashboard/documents" className="w-fit inline-block p-1 revert-light-gradient rounded-full" as="button">
          ...
          <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
            <DynamicHtmlTag
              ...
              <DynamicHtmlTag type="span" className="max-w-max text-center text-sm xl:text-base lg:py-1 pb-10 block m-auto font-semibold">
                ...
                />
                Vous n{`'`} avez pas encore enregistré de compte-rendu.
              </DynamicHtmlTag>
            </DynamicHtmlTag>
          </DynamicHtmlTag>
        ) : (
          <DynamicHtmlTag type="div" className="max-h-table overflow-auto min-w-full bg-white">
            <DynamicHtmlTag type="div" className="flex justify-between px-3 xl:mt-2">
              <DynamicHtmlTag type="div" className="flex gap-3">
                ...
                <DynamicHtmlTag type="div" className="h-5 border border-r bg-gray-500" />
            ...
            <DynamicHtmlTag type="div" className="w-full px-1 xl:px-3 py-2 relative h-full main-tab-inner overflow-hidden">
              <DynamicHtmlTag
                type="div"
                className="max-h-table overflow-auto min-w-full bg-white lg:h-[88%] lg:max-h-[15rem] xl:max-h-[32rem] table-mobile-height pb-6 lg:pb-0">
                <table className="mt-2 xl:mt-1 min-w-full bg-white">
                  <thead>
                    ...
                    {CategoryList?.map((item, index) => {
                      return (
                        <>
                          <tr
                            key={index}
                            className="cursor-pointer text-[2vw] md:text-[6px] xl:text-2xs 2xl:text-xs font-bold lg:border-b lg:border-slate-100">
                            <td className="py-1 px-1 2xl:py-3 2xl:px-3">
                              <DynamicHtmlTag type="div" className="flex lg:gap-5">
                                ...
                                <DynamicHtmlTag type="span">{item?.type?.name}</DynamicHtmlTag>
                              </DynamicHtmlTag>
                            </td>

                            <td className="py-1 px-1 2xl:py-3 2xl:px-3 hidden md:table-cell">
                            ...
                            <td className="hidden md:table-cell">
                              <DynamicHtmlTag type="span" className="line-clamp-1">
                                {item?.details?.consultationType?.name}
                              </DynamicHtmlTag>
                            </td>
                            ...
                            <td className="py-1 px-1 2xl:py-3 2xl:px-3 w-1/6">
                              <DynamicHtmlTag type="div" className="flex items-center space-x-2 lg:space-x-4 justify-center">
                                ...
                                <CustomButton as="button" onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_BASE_URL}${item?.file?.url}`)}>
                                  <CustomImage
                                    className="w-3 h-3 md:w-4 md:h-4 lg:w-3 lg:h-3 xl:w-4 xl:h-4 2xl:w-5 2xl:h-5 max-w-max"
                          ...
                          <tr key={index} className="md:hidden pb-1 border-b lg:border-slate-100">
                            <td colSpan={4} className="pb-2">
                              <DynamicHtmlTag
                                type="div"
                                className="flex justify-between items-center bg-gray-400 bg-opacity-40 rounded-lg text-[2vw] md:text-3xs font-bold w-full py-1 px-3">
                                <DynamicHtmlTag type="div" className="w-2/3">
                                  <DynamicHtmlTag type="div" className="text-gray-500">
                                    Nom du document
                                  </DynamicHtmlTag>
                                  <DynamicHtmlTag type="span" className="font-semibold text-black line-clamp-1">
                                    {item?.name}
                                  </DynamicHtmlTag>
                                </DynamicHtmlTag>
                                <DynamicHtmlTag type="div" className="w-1/3">
                                  <DynamicHtmlTag type="div" className="text-gray-500">
                                    Consultation Type
                                  </DynamicHtmlTag>
      ...
      <DeleteDocument
        ...
        paragraph={
          MedicalDocumentName
            ? `Êtes-vous sûr de vouloir supprimer ce compte rendu : ${MedicalDocumentName} ?`
            : "Êtes-vous sûr de vouloir supprimer la liste des comptes rendus sélectionnée ?"
        }
        icon={<FaTrash className="text-red-500" />}
        title={MedicalDocumentName ? "Supprimer un compte rendu" : "Supprimer les comptes rendus"}
        description="Êtes-vous sûr de vouloir supprimer ces documents ?"
      />
    </DynamicHtmlTag>
  );
};
