...
const OtherDocument = () => {
  ...
  const fetchCategoryTypeList = useCallback(async () => {
    try {
      ...
      const categoryOptions: DocumentOption[] = response
        .filter(category => category.code === "AUDO")
        .map(category => ({
          value: category.id,
          label: category.name,
          code: category.code,
  ...
  return (
    <DynamicHtmlTag type="div" className="bg-white shadow-lg rounded-lg h-full overflow-auto">
      <DynamicHtmlTag type="div" className="tabs-content rounded-t-lg pt-0" id="other-doc">
              ...
              <DynamicHtmlTag
                ...
                <table className="min-w-full bg-white lg:mt-5">
                  ...
                  {CategoryList?.map((item, index) => {
                    return (
                      <tbody key={index}>
                        <tr className="cursor-pointer text-[2vw] md:text-3xs xl:text-2xs font-bold border-b border-slate-100">
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left">
                            <DynamicHtmlTag type="div" className="flex gap-o lg:gap-5">
                              ...
                              <DynamicHtmlTag type="span">{item?.name}</DynamicHtmlTag>
                            </DynamicHtmlTag>
                          </td>
                          <td className="py-1 px-1 2xl:py-3 2xl:px-3 text-left">
      ...
      <DeleteDocument
        ...
        title={
          MedicalDocumentName ? "Supprimer un document de catégorie : Autres documents" : "Supprimer les documents de catégorie Autres documents"
        }
        paragraph={
          MedicalDocumentName
            ? `Êtes vous sur de vouloir supprimer ce document : ${MedicalDocumentName} ?`
            : "Êtes vous sur de vouloir supprimer la liste des documents?"
        }
